...
def main(args: argparse.Namespace) -> Optional[tf.keras.Model]:
    ...
    if not args.evaluate:
        ...
        model.add(tf.keras.layers.GaussianDropout(.3))
        ...
        model.add(tf.keras.layers.GaussianDropout(.2))
        model.add(tf.keras.layers.Dense(
            32,
            activation=tf.nn.tanh,
            kernel_regularizer=tf.keras.regularizers.l1_l2(l1=1e-5, l2=1e-4),
        ...
        model.add(tf.keras.layers.GaussianDropout(.1))
        ...
        model.compile(
            ...
            metrics=[tf.keras.metrics.BinaryAccuracy("accuracy"),
                     tf.keras.metrics.BinaryCrossentropy("entropy"),
