...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self._W1 = torch.empty(
            [MNIST.W * MNIST.H * MNIST.C, args.hidden_layer], requires_grad=True
        )
        nn.init.normal_(self._W1, 0, 0.1)

        self._b1 = torch.zeros([args.hidden_layer], requires_grad=True)

        self._W2 = torch.empty([args.hidden_layer, MNIST.LABELS], requires_grad=True)
        nn.init.normal_(self._W2, 0, 0.1)

        self._b2 = torch.zeros([MNIST.LABELS], requires_grad=True)

    def forward(self, x):
        x = torch.reshape(x, [x.shape[0], -1])
        x = F.tanh(x @ self._W1 + self._b1)
        return x @ self._W2 + self._b2
...
def train_loop(dataloader, model, loss_fn, step=None, writer=None):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        loss.backward()
        with torch.no_grad():
            model._W1 -= args.learning_rate * model._W1.grad
            model._b1 -= args.learning_rate * model._b1.grad
            model._W2 -= args.learning_rate * model._W2.grad
            model._b2 -= args.learning_rate * model._b2.grad
            model._W1.grad.zero_()
            model._b1.grad.zero_()
            model._W2.grad.zero_()
            model._b2.grad.zero_()
