...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        predictions = tf.keras.layers.Dense(
            train.tags.word_mapping.vocabulary_size(), activation=None
        )(sequences)
        ...
        self.compile(
            ...
            loss=self.crf_loss,
            metrics=[
                self.SpanLabelingF1Metric(
                    train.tags.word_mapping.get_vocabulary(), name="f1"
                )
            ],
        )
    ...
    def crf_loss(
        self, gold_labels: tf.RaggedTensor, logits: tf.RaggedTensor
    ) -> tf.Tensor:
        assert isinstance(
            gold_labels, tf.RaggedTensor
        ), "Gold labels given to CRF loss must be RaggedTensors"
        assert isinstance(
            logits, tf.RaggedTensor
        ), "Logits given to CRF loss must be RaggedTensors"

        class CRFCell(tf.keras.layers.AbstractRNNCell):
            def __init__(self, crf_weights, **kwargs):
                self.crf_weights = crf_weights
                super(CRFCell, self).__init__(**kwargs)

            @property
            def state_size(self):
                return self.crf_weights.shape[0]

            def call(self, inputs, states):
                prev_output = states[0]
                output = inputs + tf.vectorized_map(
                    lambda x: tf.math.reduce_logsumexp(
                        tf.expand_dims(x, 1) + self.crf_weights, axis=0
                    ),
                    prev_output,
                )
                return output, output

        state = tf.keras.layers.RNN(
            CRFCell(self._crf_weights),
            return_sequences=False,
            return_state=True,
        )(
            logits[:, 1:, :],
            initial_state=tf.squeeze(tf.reverse(logits, [1])[:, -1:, :], axis=1),
        )[
            0
        ]
        ...
        crf = (
            ...
            + tf.reduce_sum(
                tf.gather_nd(
                    self._crf_weights,
                    tf.stack([gold_labels[:, :-1], gold_labels[:, 1:]], axis=2),
                ),
                axis=1,
            )
            - tf.math.reduce_logsumexp(state, axis=1)
        )

        return tf.reduce_mean(-crf, axis=0)

    def crf_decode(self, logits: tf.RaggedTensor) -> tf.RaggedTensor:
        assert isinstance(
            logits, tf.RaggedTensor
        ), "Logits given to CRF decoding must be RaggedTensors"
        ...
        # to a ragged tensor.
        preds, _ = tfa.text.crf_decode(
            logits.to_tensor(),
            transition_params=self._crf_weights,
            sequence_length=logits.row_lengths(),
        )
        predictions = tf.RaggedTensor.from_tensor(preds, lengths=logits.row_lengths())
    ...
    def train_step(self, data):
        ...
        return {
            "loss": metric.result() for metric in self.metrics if metric.name == "loss"
        }
    ...
    class SpanLabelingF1Metric(tf.metrics.Metric):
        ...
        def __init__(self, tags, name="span_labeling_f1", dtype=None):
            ...
            self._counts = self.add_weight(
                "counts", shape=[3], initializer=tf.initializers.Zeros(), dtype=tf.int64
            )
        ...
        def classify_spans(self, y_true, y_pred, sentence_limits):
            ...
            return np.array(
                [
                    len(spans_true & spans_pred),
                    len(spans_pred - spans_true),
                    len(spans_true - spans_pred),
                ],
                np.int64,
            )
        ...
        def update_state(self, y, y_pred, sample_weight=None):
            assert isinstance(y, tf.RaggedTensor) and isinstance(
                y_pred, tf.RaggedTensor
            )
            assert sample_weight is None, "sample_weight currently not supported"
            counts = tf.numpy_function(
                self.classify_spans, (y.values, y_pred.values, y.row_limits()), tf.int64
            )
        ...
        def result(self):
            ...
            return tf.math.divide_no_nan(
                tf.cast(2 * tp, tf.float32), tf.cast(2 * tp + fp + fn, tf.float32)
            )
