...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        whole_seq_out = LAYERS[args.rnn_cell](
            args.rnn_cell_dim, return_sequences=True, return_state=False
        )(sequences)
        ...
        if args.hidden_layer:
            hidden = tf.keras.layers.Dense(args.hidden_layer, activation="relu")(
                whole_seq_out
            )
            predictions = pred(hidden)
        else:
            predictions = pred(whole_seq_out)
