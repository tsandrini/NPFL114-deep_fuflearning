...
from typing import Tuple, Dict, Iterator, Optional, List, Callable, Union
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        for hidden_layer in args.hidden_layers:
            ...
            self.blocks.append(nn.Sequential(nn.Linear(in_dim, out_dim), nn.ReLU()))
            in_dim = out_dim
...
class EnsembleModel(nn.Module):
    ...
    def forward(self, x):
        return torch.mean(
            torch.stack([model(x) for model in self.models], dim=-1), dim=-1
        )
    ...
    for model in range(args.models):
        ...
        containers.append(
            ModelContainer(
                ...
                metrics={"acc": torchmetrics.Accuracy()},
                update_metrics_fn=update_metrics_fn,
                writer=writer,
            )
        )
        print(
            "Training model {}: ".format(model + 1), end="", file=sys.stderr, flush=True
        )
        containers[-1].fit(train, epochs=args.epochs)
        print("Done", file=sys.stderr)

    print("\nEnsembling\n---------\n")
    individual_accuracies, ensemble_accuracies = [], []
    for model in range(args.models):
        individual_accuracy = containers[model].evaluate(test)["acc"]
        if model > 0:
            ensemble_model = EnsembleModel(args, models[: model + 1])
            ensemble_container = ModelContainer(
                args,
                ensemble_model,
                metrics={"acc": torchmetrics.Accuracy()},
                update_metrics_fn=update_metrics_fn,
            )
            ensemble_accuracy = ensemble_container.evaluate(test)["acc"]
        else:
